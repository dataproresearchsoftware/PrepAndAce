...
import fetchVerticalNav from 'src/store/navigation/vertical'
...
const defaultSuggestionsData = [
  {
    ...
    suggestions: [
      ...
      },
      {
        ...
        suggestion: 'Inventory Purchase',
        link: '/inventory/transactions/purchase/list'
      },
      {
        icon: 'tabler:clipboard-list',
        suggestion: 'Inventory Sales',
        ...
        <Grid item xs={12} key={index}>
          ...
          <Grid container spacing={6} mt={1}>
            {item.suggestions.map((suggestionItem, index2) => (
              <Grid item key={index2} xs={12} md={4}>
                <Box
                  component={Link}
...
const AutocompleteComponent = ({ hidden, settings }) => {
  ...
  useEffect(() => {
    dispatch(fetchVerticalNav()).then(resp => {
      const data = resp.payload

      const searchMenu = data?.map(i => ({
        id: i.pageId,
        icon: i.icon,
        title: i.title,
