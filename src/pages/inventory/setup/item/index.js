...
import { updateItem, deleteItem, getItem } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/inventory/setup/item/static-data'
import { dateconvert, handleSearch, isAllowed } from 'src/@core/utils'
import { print } from 'src/@core/utils'
import usePermission from 'src/@core/hooks/usePermission'
import { ItemForm } from 'src/views/pages/inventory/setup/item/form'
...
const Item = () => {
  ...
  useEffect(() => {
    dispatch(getItem()).then(response => {
      if (response?.error) {
  ...
  const handleEdit = row => {
    setValue('itype', { value: row.itype, label: row.itname })
    setValue('itemcode', row.itemcode)
    setValue('iname', row.iname)
    setValue('oqty', row.oqty)
    setValue('orate', row.orate)
  ...
  const handleConfirm = ({ data }) => {
    setDeleteStates({ itype: data.itype, itname: data.itname, icode: data.icode, iname: data.iname })
    setStates({ ...states, modalForm: 'd', isOpenModal: true })
  }

  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteItem({ icode: deleteStates.icode })).then(response => {
      ...
      {states.modalForm === 'd' ? (
        ...
        >
          <p style={{ textDecoration: 'underLine', my: 3, textAlign: 'center' }}>
            <strong>
              <span>Item Type: {deleteStates.itname}</span>
            </strong>
          </p>
          <p style={{ textDecoration: 'underLine', my: 3, textAlign: 'center' }}>
            <strong>
              <span>Item Code: {deleteStates.icode}</span>
            </strong>
          </p>
          <p style={{ textDecoration: 'underLine', my: 3, textAlign: 'center' }}>
            <strong>
              <span>Item Name: {deleteStates.itname}</span>
            </strong>
          </p>
        </DeleteModal>
      ) : (
