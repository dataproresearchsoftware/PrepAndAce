...
import { getCustomer, updateCustomer, deleteCustomer } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/inventory/setup/customer/static-data'
import { dateconvert, handleSearch, isAllowed } from 'src/@core/utils'
import { print } from 'src/@core/utils'
import usePermission from 'src/@core/hooks/usePermission'
import { CustomerForm } from 'src/views/pages/inventory/setup/customer/form'
...
const Customer = () => {
  ...
  const handleEdit = row => {
    setValue('customercode', row.customercode)
  ...
  const handleConfirm = ({ data }) => {
    setDeleteStates({ customercode: data.customercode, customername: data.customername })
    setStates({ ...states, modalForm: 'd', isOpenModal: true })
  }

  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteCustomer({ customercode: deleteStates.customercode })).then(response => {
      ...
      {states.modalForm === 'd' ? (
        ...
        >
          <p style={{ textDecoration: 'underLine', my: 3, textAlign: 'center' }}>
            <strong>
              <span>customer: {deleteStates.customercode}</span>
              <span style={{ paddingLeft: 30 }}>Name: {deleteStates.customername}</span>
            </strong>
          </p>
        </DeleteModal>
      ) : (
