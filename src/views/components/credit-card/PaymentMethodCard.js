...
import { CardCVCTextInput, CreditCardTextInput } from './credit-card'
...
export const PaymentMethodCard = ({ isOpen, setIsOpen }) => {
  // ** States
  ...
  const [states, setStates] = useState({ isSubmit: false })
  ...
  const submitPayment = async () => {
    setStates({ isSubmit: true })
    await setTimeout(() => {
      setStates({ isSubmit: false })
      toast.success('Payment Successful', { duration: 1000, position: 'top-center' })
    }, 2000)
    await setTimeout(() => {
      setIsOpen(false)
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} sm={3}>
                  <CustomTextField
                    ...
                    InputProps={{
                      inputComponent: CardCVCTextInput
                    }}
                  />
                </Grid>
                {/* <Grid item xs={12}>
